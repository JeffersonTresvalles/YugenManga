...
class _DownloadsScreenState extends State<DownloadsScreen> {
  ...
  /// Get the theme provider's accent color
  ...
  Future<void> _refreshAllGenres() async {
    ...
    try {
      ...
      for (final entry in mangaIdToTitle.entries) {
        ...
        await _dbHelper.updateGenreByMangaId(entry.key, genre);
      }
    } finally {
      if (mounted) {
  ...
  /// Deletes all downloaded chapters for the given manga and removes
  /// the local files from device storage.
  Future<void> _deleteManga(String mangaId, String title) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: Theme.of(context).brightness == Brightness.dark
            ...
            : Colors.white,
        title: Text('Delete Manga', style: GoogleFonts.nunito(fontWeight: FontWeight.w800)),
        content: Text(
          'Are you sure you want to delete all downloaded chapters for "$title"? This action cannot be undone.',
    ...
    if (confirmed == true) {
      // Get all downloads for this manga to delete files
      final downloads = await _dbHelper.getDownloads();
      final mangaDownloads = downloads.where((d) => d['mangaId'] == mangaId).toList();

      // Delete from DB
      await _dbHelper.deleteDownloadsByMangaId(mangaId);
        ...
        if (localPath != null) {
          ...
          if (dir.existsSync()) {
            try {
              ...
              debugPrint('Failed to delete directory: $localPath, $e');
            }
          }
        }
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Deleted "$title" from downloads'),
  ...
  Color _getGenreColor(String genre) {
    ...
    return _getAccentColor(context);
  }
            ...
            child: FutureBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return Center(
                    child:
                        CircularProgressIndicator(color: _getAccentColor(context)),
                  );
                }
                ...
                return RefreshIndicator(
                  ...
                  color: accentColor, // Use accentColor for RefreshIndicator
                  backgroundColor:
                              ...
                              child: Container(
                                ...
                                child: Icon(
                                  _selectedGenre == 'All'
                                      ...
                                      : Icons.filter_list_off_rounded, // Use accentColor for empty state icon
                                  size: 44,
  ...
  Widget _buildMangaCard(Map<String, dynamic> manga) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (_) => ChapterListOfflineScreen(
                        mangaTitle: title,
                        mangaId: manga['mangaId']?.toString(),
                      )))
          .then((_) => _loadDownloads()),
      onLongPress: () {
        final mangaId = manga['mangaId']?.toString();
        if (mangaId != null) {
          _deleteManga(mangaId, title);
        }
      },
      child: Column(
              ...
              child: Stack(
                ...
                children: [
                  ...
                  Positioned(
                    ...
                    right: 8,
                    child: Container(
                      padding: const EdgeInsets.symmetric( // Use accentColor for chapter count badge
  ...
  Widget _getCoverWidget(String? dbPath, String? localPath) {
    ...
    if (coverFile == null && localPath != null) {
      try {
        final dir = Directory(localPath); // Current chapter folder
        final mangaDir = dir.parent;      // Manga root folder
        
        // Check for the dedicated cover file first
        final dedicatedCover = File('${mangaDir.path}/cover.jpg');
        if (dedicatedCover.existsSync()) return Image.file(dedicatedCover, fit: BoxFit.cover);
        ...
        if (dir.existsSync()) {
          final files = dir
              ...
              .where((f) =>
                  ...
                  RegExp(r'\d+$').hasMatch(p.basename(f.path))) // Recognize extension-less "Ghost Mode" files
              .toList();
          if (files.isNotEmpty) {
