    ...
    while (true) {
      ...
      try {
        ...
        if (customPath != null && customPath != 'Default (Internal)') {
          folderPath = '$customPath/${task.mangaId}/${task.chapterId}';
        } else {
          final directory = await getApplicationDocumentsDirectory();
          folderPath =
              '${directory.path}/downloads/${task.mangaId}/${task.chapterId}';
        }
        ...
        for (int i = task.resumeFromPage; i < task.imageUrls.length; i++) {
          ...
          // Zero-pad the filename (e.g. "003.jpg") so file system ordering
          // matches page order without additional sorting.
          final savePath =
              '$folderPath/${(i + 1).toString().padLeft(3, '0')}.jpg';
          ...
          final p = (i + 1) / task.imageUrls.length;
          _updateTask(task.chapterId, progress: p);
          // Only invoke the external progress callback for the originally requested chapter.
          if (task.chapterId == currentId) onProgressForCurrent(p);
        }
        ...
        // Keep the error visible briefly, then remove the task.
