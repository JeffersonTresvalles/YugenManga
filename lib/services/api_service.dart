...
class ApiService {
  ...
  static const String safeFilter =
      '&contentRating[]=safe&contentRating[]=suggestive&includes[]=cover_art&includes[]=author&includes[]=artist';
  ...
  Future<List<Manga>> fetchMangaList() async {
    final response = await http.get(
      Uri.parse('$baseUrl/manga?limit=20$safeFilter'),
  ...
  /// Fetches a single manga by its [mangaId].
  /// Useful for refreshing "placeholder" manga objects from the Favorites screen.
  Future<Manga?> fetchMangaById(String mangaId) async {
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/manga/$mangaId?$safeFilter'),
      ).timeout(const Duration(seconds: 15));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
  ...
  Future<List<String>> fetchPagesByChapterId(String chapterId) async {
    try {
      final response =
          await http.get(Uri.parse('$baseUrl/at-home/server/$chapterId')).timeout(const Duration(seconds: 15));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final String host =
  ...
  Future<List<Manga>> searchManga(String title) async {
    ...
    final queryString =
        'title=$encodedTitle&limit=15&includes[]=cover_art&contentRating[]=safe&contentRating[]=suggestive';
    final uri = Uri.parse('$baseUrl/manga?$queryString');
